...
for j = 1:length(dim_hidden)
    ...
    for k = 1:dim_hidden(j)
        ...
        if abs(Y_max_curr_layer(k)) > abs(Y_min_curr_layer(k))
            ...
            ineq_constraints{icount,1} = (x_curr_layer(k) - (X_max_curr_layer(k)/Y_max_curr_layer(k))*v{j}(k))*(v{j}(k) - x_curr_layer(k)); ineq_constraints{icount,2} = [j,k]; icount = icount + 1; 
        
        elseif abs(Y_max_curr_layer(k)) < abs(Y_min_curr_layer(k))

            ineq_constraints{icount,1} = (x_curr_layer(k) - (X_min_curr_layer(k)/Y_min_curr_layer(k))*v{j}(k))*(v{j}(k) - x_curr_layer(k)); ineq_constraints{icount,2} = [j,k]; icount = icount + 1;
        ...
        % Pre-processing bounds
        ineq_constraints{icount,1} = (x_curr_layer(k) - X_min_curr_layer(k))*(-x_curr_layer(k) + X_max_curr_layer(k)); ineq_constraints{icount,2} = [j,k]; icount = icount + 1; 
        %ineq_constraints{icount,1} = (x_curr_layer(k) - X_min_curr_layer(k)); ineq_constraints{icount,2} = [j,k]; icount = icount + 1; 
        %ineq_constraints{icount,1} = (-x_curr_layer(k) + X_max_curr_layer(k)); ineq_constraints{icount,2} = [j,k]; icount = icount + 1; 
