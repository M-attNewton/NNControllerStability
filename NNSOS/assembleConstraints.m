...
elseif sos_type == 1
    ...
    k = 1;
    for i = 1:length(vars)
        for j = length(constraints.varname)
            if strcmp(constraints.varname{j},vars.varname{i}) 
                temp(k) = vars(j);
                k = k + 1;
            end
        end
...
elseif sos_type == 5
    if node_number(1) > 1
        if node_number(1) + 1 <= length(dim_hidden)
            vars_layer = (dim_in + 1 + sum(dim_hidden(1:(node_number(1) - 2)))):(dim_in + sum(dim_hidden(1:(node_number(1)))) - 0);
        else
            vars_layer = (dim_in + 1 + sum(dim_hidden(1:(node_number(1) - 2)))):(dim_in + sum(dim_hidden(1:end)) - 0);
        end
    elseif node_number(1) == 1
