...
%UPDATE 14/07/21 reduced size of constraints
%ineq_constraints = cell(length(Ip) + length(In) + 2*length(Ipn),2);
%eq_constraints = cell(1*length(Ip) + 1*length(In) + length(Ipn),2);
%ineq_constraints = cell(0*length(Ip) + 0*length(In) + 2*length(Ipn),2);
eq_constraints = cell(1,2);
ineq_constraints = cell(1,2);
