...
dotz1 = z(2); %theta
dotz2 = (mass*grav*leng*(z(1) - z(3)) - mu*z(2) + v_out)/(mass*leng^2); %dot theta
...
elseif abs(u_max(1)) < abs(u_min(1))
    grad_z3 = (umin(1) - sin(u_min(1)))/abs(u_min(1));
else
    grad_z3 = (u_max(1) - sin(u_max(1)))/abs(u_max(1));
end
...
domain1 = [-0.3, 0.3, -0.7, 0.7];
[C3,h3] = pcontour(SOL,0.04,domain1,'b',[300, 300]);
