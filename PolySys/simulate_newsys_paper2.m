...
for z1i = 0
    for z2i = -5:2:5 
        ...
        if any(zout(end,1) > 3) || any(zout(end,2) > 3) || any(zout(end,3) > 3)
            ...
            plot(zout_temp(:,2),zout_temp(:,3),'r','LineWidth',2)
            %z1_temp = all(abs(zout(end,1)) < 3)
            %axis([-3, 3, -3, 3, -3, 3])
            hold on
        else
            plot(zout(:,2),zout(:,3),'b','LineWidth',2)
        end
		hold on
        end
    end
...
syms z_1 z_2 z_3
%SOL_3 = 1.4648*z_1^2 - 0.97158*z_1*z_2 + 0.66135*z_2^2 - 4.1378*z_1 + 2.5674*z_2 + 6.9403;

%SOL_3 = 1.5195*z_1^2 - 1.01*z_1*z_2 + 0.68856*z_2^2 - 4.2912*z_1 + 2.6733*z_2 + 7.21;
% 
% domain1 = [-4,4,-4,4];
% fmn = fcontour(SOL_3,domain1, 'LineColor', 'k', 'LineWidth', 3);
% fmn.LevelList = 4.5;

% SOL_2 = 1.5195*z_1^2 + 4.2912*z_1*z_3 + 7.21*z_3^2;
% domain1 = [-4,4,-4,4];
% fmn = fcontour(SOL_2,domain1, 'LineColor', 'k', 'LineWidth', 3);
% fmn.LevelList = 4.5;

SOL_1 = 0.68856*z_2^2 - 2.6733*z_2*z_3 + 7.21*z_3^2;
domain1 = [-4,4,-4,4];
fmn = fcontour(SOL_1,domain1, 'LineColor', 'k', 'LineWidth', 3);
fmn.LevelList = 4;
...
%f  =  1.5195*z_1^2 - 1.01*z_1*z_2 + 4.2912*z_1*z_3 + 0.68856*z_2^2 - 2.6733*z_2*z_3 + 7.21*z_3^2 - 4.5;
